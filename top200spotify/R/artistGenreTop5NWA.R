source('getData.R')
source('top200spotify/R/getData.R')
list.files('C:/Users/ltert/OneDrive/Documentos/top200spotify')

market <- readRDS('C:/Users/ltert/OneDrive/Documentos/top200spotify/market_genre_01072017.Rda')
saveRDS(market, 'C:/Users/ltert/OneDrive/Documentos/top200spotify/market_genre_01072017_updated.Rds')

top200 <- readRDS('C:/Users/ltert/OneDrive/Documentos/top200spotify/top200clean_01072017.Rda')
saveRDS(top200, 'C:/Users/ltert/OneDrive/Documentos/top200spotify/top200clean_01072017_updated.Rds')

market.freq <- readRDS('C:/Users/ltert/OneDrive/Documentos/top200spotify/market_genre_freq_01072017.Rda')
saveRDS(market.freq, 'C:/Users/ltert/OneDrive/Documentos/top200spotify/market_genre_freq_01072017_updated.Rds')

artist <- readRDS('C:/Users/ltert/OneDrive/Documentos/top200spotify/artists_genre_01072017.Rda')
saveRDS(artist, 'C:/Users/ltert/OneDrive/Documentos/top200spotify/artists_genre_01072017_updated.Rds')

artist.info <- readRDS('C:/Users/ltert/OneDrive/Documentos/top200spotify/artists_info_01072017.Rda')
saveRDS(artist.info, 'C:/Users/ltert/OneDrive/Documentos/top200spotify/artists_info_01072017_updated.Rds')

artist.area <- readRDS('C:/Users/ltert/OneDrive/Documentos/top200spotify/artists_area_mbid_01072017.Rda')
saveRDS(artist.area, 'C:/Users/ltert/OneDrive/Documentos/top200spotify/artists_area_mbid_01072017_updated.Rds')
...
market.genre.u <- na.omit(market); head(market)
...
library(igraph)
market.top5genre.g <- graph_from_biadjacency_matrix(market.top5genre.am)
market.top5genre.projec <- bipartite_projection(market.top5genre.g)
market.top5genre.projec
...
# Yep!
market.top5genre.edges <- as.data.frame(as_edgelist(market.top5genre.projec$proj1)); head(market.top5genre.edges)
...
ggplot(as.data.frame(market.top5genre.deg.dist), 
  ...
  geom_point(colour=colors[2],size = 2) +
  theme_minimal() +
...
plot(x=0:max(degree(gnp_proj1)), y=degree_distribution(gnp_proj1), pch=19, 
     cex=1.2, col=colors[2], xlab="Grau", ylab="Frequência Acumulada",
     xaxt='n'); axis(side = 1, at=seq(0,25,5))

# Projection 2
vertices.projec2 <- V(market.top5genre.projec$proj2)
...
colors.c <- brewer.pal(9, "BuGn")[membership(market.genre.comunities)]
...
top200complete <- readRDS('C:/Users/ltert/OneDrive/Documentos/top200spotify/top200clean_01072017.Rda')
...
head(eachMk.merged)
countries <- read.csv('C:/Users/ltert/OneDrive/Documentos/top200spotify/pais_sigla.csv')
colnames(eachMk.merged)
colnames(countries) <- c('mk', 'country')
countries$mk <- tolower(countries$mk)
head(countries)
...
library(crayon)
library(ggplot2)
length(eachMk.merged$mk)
length(eachMk.merged$freq)
ggplot(eachMk.merged, aes(x = mk, y = freq)) + 
  geom_bar(stat = 'identity')
# Example ggplot with correct mappings
ggplot(eachMk.merged, aes(x = mk, y = freq, fill = tipo, colour = tipo)) + 
...
par(mar=c(0,2,2,0))
strength.genre <- rep(1:5, length.out = vcount(market.top5genre.projec$proj2))
plot(market.top5genre.projec$proj2, 
     main="Degree", vertex.size=V(market.top5genre.projec$proj2)$size*strength.genre/10)

# How many genres are original? #############################
origin_genre <- read.csv('origin_genre.csv')
class(origin_genre)
head(origin_genre)
unique(origin_genre$mk) # 99
head(origin_genre)
unique(droplevels(top5Genre.eachMk$market))
origin_genre <- merge(origin_genre, countries, by='mk')
a <- origin_genre[,c(1,2)]
a
a$mk <- tolower(a$mk)
a
...
diagonal <- data.frame(to = unique(market.genre.edges$to),
                       ...
                       sim = 1)
head(market.genre.edges)
head(market.genre.edges.oh)
head(diagonal)
# Mash 'em all together
full <- bind_rows(market.genre.edges, market.genre.edges.oh); head(full)
full
